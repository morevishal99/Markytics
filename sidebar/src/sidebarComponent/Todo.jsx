    ...
    return (
        <>
            ...
            <div style={{ width: "70%", margin: "auto" }}>
                ...
                {/* <h1>{todoValue}</h1> */}
                {todo?.map((item, index) =>
                    <div style={{ display: "flex", gap: "20px", marginTop: "5px", border: "1px solid black", boxShadow:"10px 10px 100px red", borderRadius: "10px", padding: "5px" }}>
                        <h4 style={{color:"red",marginTop:"7px"}}>{index + 1} :- {item.todo.toUpperCase()}</h4>
                        <h2 style={{ backgroundColor: "aliceblue", margin: "2px", borderRadius: "6px",padding:"2px" }} onClick={() => handleDelete(index)} >Delete</h2>
                    </div>
                )}
            </div>
        </>
